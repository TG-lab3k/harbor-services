...
import (
	...
	"github.com/okok/harbor-services/internal/platform/middleware"
...
// tenantGate lazily forwards AppGate calls to AppService (set after construction).
...
func main() {
	...
	authSvc := authapp.NewService(authapp.Deps{
		...
		Encryptor:      encryptor,
		JWT:            jwtSvc,
		EmailSender:    authapp.LogEmailSender{},
		RateLimiter:    authapp.NewMemoryRateLimiter(),
		StateStore:     authapp.NewMemoryOAuthStateStore(),
		BaseURL:        cfg.BaseURL,
	})
	...
	r := gin.New()
	r.Use(middleware.Recover(), middleware.RequestID(), middleware.CORS(), middleware.AccessLog())
